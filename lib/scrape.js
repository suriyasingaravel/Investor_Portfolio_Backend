...
// Always force English labels so parsing is stable
function gfUrl(symbol, exchange) {
  // exchange is NSE or BOM (for BSE)
...
// Forgiving metric scan (covers multiple label spellings and DOM layouts)
function getMetric($, labels) {
  ...
  // Sliding window: after a label, scan the next few tokens for a number
...
async function scrapeFundamentalsOnce(symbol, exchange) {
  ...
  // Google uses a few variants for these labels
  const PE_LABELS = [
    "P/E ratio",
    "P/E ratio (TTM)",
